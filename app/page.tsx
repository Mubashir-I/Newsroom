        ...
        <div>
          ...
          {isLoading ? (
            ...
            <div className="space-y-10">
              {/* Featured Article */}
              {featured && (
                <Link href={`/articles/${featured._id}`} className="group block">
                  <div className="grid md:grid-cols-2 gap-0 border border-zinc-800 rounded-xl overflow-hidden hover:border-zinc-600 transition-all duration-300 bg-zinc-950">
                    <div className="h-64 md:h-auto relative bg-zinc-900 min-h-[220px]">
                      {featured.coverImage ? (
                        <Image src={featured.coverImage} alt={featured.title} fill className="object-cover group-hover:scale-[1.02] transition-transform duration-500" />
                      ) : (
                    ...
                    <div className="p-7 md:p-10 flex flex-col justify-center">
                      <div className="flex items-center gap-3 mb-5">
                        <span className="px-2.5 py-1 bg-zinc-900 border border-zinc-700 text-zinc-300 text-[10px] font-bold uppercase tracking-widest rounded-sm">
                          {featured.category}
                        </span>
                        <span className="text-zinc-600 text-xs flex items-center gap-1">
                          <Clock className="w-3 h-3" />
                          {new Date(featured.createdAt).toLocaleDateString('en-US', { month: 'short', day: 'numeric' })}
                        </span>
                      </div>
                      <h2 className="text-2xl md:text-3xl font-bold text-zinc-100 leading-tight mb-3 group-hover:text-white transition-colors">
                        {featured.title}
                      </h2>
                      ...
                      <div className="flex items-center gap-3 mt-auto">
                        ...
                        <span className="text-xs font-semibold text-zinc-400">{featured.author?.username}</span>
                        <span className="ml-auto text-xs text-zinc-500">{featured.likes?.length || 0} likes</span>
                      </div>
                    </div>
                  </div>
                </Link>
              ...
              {rest.length > 0 && (
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
                  {rest.map((article) => (
                    <Link key={article._id} href={`/articles/${article._id}`} className="group block bg-zinc-950 border border-zinc-800 rounded-xl overflow-hidden hover:border-zinc-600 transition-all duration-300">
                      <div className="h-44 relative bg-zinc-900">
                        {article.coverImage ? (
                          <Image src={article.coverImage} alt={article.title} fill className="object-cover opacity-80 group-hover:opacity-100 group-hover:scale-[1.02] transition-all duration-500" />
                        ) : (
                          <div className="absolute inset-0 bg-gradient-to-br from-zinc-800 to-zinc-900 flex items-center justify-center">
                      ...
                      <div className="p-5">
                        ...
                        <h3 className="text-base font-bold text-zinc-100 mb-2 leading-tight line-clamp-2 group-hover:text-white transition-colors">
                          {article.title}
                        </h3>
                        ...
                        <div className="flex items-center justify-between border-t border-zinc-900 pt-3">
                          <span className="text-xs font-medium text-zinc-500">{article.author?.username}</span>
                          <span className="text-xs text-zinc-600">{article.likes?.length || 0} likes</span>
                        </div>
                      </div>
                    </Link>
                  ))}
